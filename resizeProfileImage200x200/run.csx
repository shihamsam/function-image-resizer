...
static string profilesContainerName = System.Environment.GetEnvironmentVariable("profilesContainerName");
...
public static async Task Run(EventGridEvent myEvent, Stream inputBlob, TraceWriter log)
{
    ...
    blobName = String.Format("{0}_200x200{1}", Path.GetFileNameWithoutExtension(blobName),  Path.GetExtension(blobName));
    ...
    CloudBlobContainer container = blobClient.GetContainerReference(profilesContainerName);
